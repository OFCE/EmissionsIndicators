...
To obtain the total emissions attributed to the producer or to demand, the emissions coefficients are multiplied by the corresponding economic volume: $S_{volume}=S\hat{X}$, $S_{VA,volume}=S\hat{V}$ and $M_{volume}=M\bf{\hat{y}$ ($\bf{\hat{y}$ is)
...
EU_groupes_pays_par.pib <- table_EU_eurostat %>%
  ...
  theme(axis.text.x = element_text(angle = 30, size=5, vjust = 1, hjust=1),
        ...
        panel.grid.major.y=element_line(color="gray",size=0.5,linetype = 2),
        plot.margin = unit(c(10,5,5,5), "mm"))+
  labs(title="Emissions (par unite pib)",
...
ggarrange(EU_pays, EU_pays_eurostat,
          common.legend = TRUE,
          ncol = 1, nrow = 2)
```

...
This report rather situates itself on descriptive grounds. It includes a summary of the state of the art in terms of approaches and methods, as well as the computations of three indicators.
...
This litterature review aims at presenting the various methods that can be used for carbon accounting, with the underlying question of how to use them to solve market failures (externalities).
...
Economic and demographic data from Eurostat was added to custom the indicators and graphs and allow to draw conclusions more easily.
...
# Visualisation and analysis of the résults
...
monde_secteurs_VA=table_monde %>% 
  ...
  theme(axis.text.x = element_text(angle = 25, size=4, vjust = 1, hjust=1),
        ...
        panel.grid.major.y=element_line(color="gray",size=0.5,linetype = 2),
        plot.margin = unit(c(10,5,5,5), "mm"))+
  labs(title="Décomposition de l'impact environnemental par composante de la VA",
       x ="Secteurs", y = "Impact GES (Gt CO2eq)",
...
monde_secteurs_VA_share=table_monde %>% 
  ...
  theme(axis.text.x = element_text(angle = 25, size=4, vjust = 1, hjust=1),
        ...
        panel.grid.major.y=element_line(color="gray",size=0.5,linetype = 2),
        plot.margin = unit(c(10,5,5,5), "mm"))+
  labs(title="Part des composantes de la VA dans l'impact environnemental",
       x ="Secteurs", y = "Impact GES (Gt CO2eq)",
       fill="Rémunération de :") +
  scale_fill_manual(labels = c("Taxes", "Travail","Capital (net)","Capital (dépréciation)"), 
                    values = c("yellow", "tomato","slateblue1","slateblue"))
...
```{r monde pays, echo=FALSE, fig.cap="A CHANGER SELON CHOIX REGIONS", fig.height=8, fig.width=6}
ggarrange(repartition_pays, monde_pays2,
...
ggarrange(repartition_secteurs, monde_secteurs,
          nrow = 2, ncol = 1, common.legend = TRUE)
```
...
table_EU_ponderation <- readRDS(str_c(path_results_tables, "IO_ponderation_all_",br_pays,"_",br,".rds"))
```
...
data= table_EU_ponderation %>% 
  ...
  pivot_longer(
    ...
    names_to = "composante",
    values_to = "impact")
EU_secteurs_VA_norm <-data%>%ggplot( 
    aes(x= produits, 
        y = impact,
        fill = composante,
        alpha=0.9)) +
...
EU_groupes_pays <- table_EU_ponderation %>%
  ...
  theme(axis.text.x = element_text(angle = 30, size=5, vjust = 1, hjust=1),
        ...
        plot.margin = unit(c(10,5,5,5), "mm"))+
  labs(title="Impacts",
       x ="Région ou pays", y = "Impact GES (kt CO2eq)",
       fill="Indicateur") +
  scale_fill_manual(
...
EU_groupes_pays_pondere.pib <- table_EU_ponderation %>%
  mutate(level_income = ifelse(PIB.hab < 20000, "<20,000€",
  ...
  theme(axis.text.x = element_text(angle = 30, size=5, vjust = 1, hjust=1),
        ...
        panel.grid.major.y=element_line(color="gray",size=0.5,linetype = 2),
        plot.margin = unit(c(10,5,5,5), "mm"))+
  labs(title="Impacts (ponderes par unite pib)",
       x ="Région ou pays", y = "Impact GES (kt CO2eq)",
       fill="Indicateur") +
  scale_fill_manual(
    labels = c("Demande", "Production","VA"), 
...
EU_groupes_pays_pondere.pop <- table_EU_ponderation %>%
  mutate(level_income = ifelse(PIB.hab < 20000, "<20,000€",
                            ifelse(PIB.hab >= 20000 & PIB.hab <= 35000, "20,000-35,000 €",
                                   ">35,000 €"))) %>% 
  #filter(nom_pays != "Reste du monde") %>%
  ...
  theme(axis.text.x = element_text(angle = 30, size=5, vjust = 1, hjust=1),
        ...
        panel.grid.major.y=element_line(color="gray",size=0.5,linetype = 2),
        plot.margin = unit(c(10,5,5,5), "mm"))+
  labs(title="Impacts (ponderes par population)",
       x ="Région ou pays", y = "Impact GES (kt CO2eq)",
       fill="Indicateur") +
  scale_fill_manual(
    labels = c("Demande", "Production","VA"), 
...
EU_pays_pondere=table_EU_ponderation %>% 
  ...
  theme(axis.text.x = element_text(angle = 25, size=8, vjust = 1, hjust=1),
        ...
        panel.grid.major.y=element_line(color="gray",size=0.5,linetype = 2),
        plot.margin = unit(c(10,5,5,5), "mm"))+
  labs(title="Impacts pondérés par la population",
...
```{r monde secteurs va, echo=FALSE, fig.height=8, fig.width=6, warning=FALSE, dev="png", dpi=400}
...
```{r groupes pays eu pib, echo=FALSE, fig.height=8, fig.width=6, message=FALSE, warning=FALSE}
ggarrange(EU_groupes_pays, EU_groupes_pays_pondere.pib,
          nrow = 2, ncol = 1, common.legend = TRUE)
```

```{r groupes pays eu pop, echo=FALSE, fig.height=8, fig.width=6, message=FALSE, warning=FALSE}
ggarrange(EU_groupes_pays, EU_groupes_pays_pondere.pop,
          nrow = 2, ncol = 1, common.legend = TRUE)
```

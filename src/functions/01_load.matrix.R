                        ...
                        path_out = path_out)
{
  ...
  if (satellite == TRUE){
    ...
    # Row names
    df_mat_row.iso <- df_mat[,1][-1:-3] |> as.character() #extraction des impacts F
    df_mat <- as.matrix(df_mat)[-1:-3,-1] 
  ...
  # Col names
  df_mat_col.iso <- df_mat[1,][-1:-2] |> as.character() #extraction du nom des pays de mat
  df_mat_col.sec <- df_mat[2,][-1:-2] |> as.character() #extraction des noms de secteur de mat
  df_mat_col.iso_sec <- str_c(col_index,df_mat_col.iso,"_",df_mat_col.sec)
  
  # Row names
  df_mat_row.iso <- df_mat[,1][-1:-3] |> as.character() #extraction du nom des pays de mat
  df_mat_row.sec <- df_mat[,2][-1:-3]  |> as.character() #extraction des noms de secteur de mat
  ...
  df_mat <- as.matrix(df_mat)[-1:-3,-1:-2] # on ne garde que les valeurs numeriques de mat et plus les noms de ligne et colonnes
  ...
  df_mat <- matrix(df_mat,length(df_mat_row.iso),length(df_mat_col.iso_sec),
                   dimnames=list(df_mat_row.iso,df_mat_col.iso_sec)) |> `rownames<-`(df_mat_row.iso_sec)
